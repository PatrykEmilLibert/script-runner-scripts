...
class CSVToExcelApp(ctk.CTk):
    ...
    def import_and_save(self):
        """Importuje dane z CSV i zapisuje je do osobnych plików Excela, każdy z dwoma arkuszami."""
        ...
        for i, csv_file in enumerate(csv_files):
            try:
                ...
                # Generowanie nazwy pliku Excela na podstawie przedrostka nazwy pliku CSV
                ...
                # Stwórz obiekt ExcelWriter do zarządzania wieloma arkuszami w *jednym* pliku Excela
                with pd.ExcelWriter(excel_full_path, engine='openpyxl') as writer:
                    # Zapisz pełne dane CSV do pierwszego arkusza
                    df.to_excel(writer, sheet_name='Dane CSV', index=False)
                    ...
                    if 'cat' in df.columns:
                        ...
                        # Stwórz DataFrame z unikalnymi kategoriami i pustą kolumną "cat id"
                        categories_df = pd.DataFrame({
                            'Kategoria': unique_categories,
                            'cat id': '' # Zmieniono nagłówek z "Google id" na "cat id"
                        })

                        # Zapisz unikalne kategorie do drugiego arkusza o nazwie "mapowanie"
                        categories_df.to_excel(writer, sheet_name='mapowanie', index=False)
                    else:
                        messagebox.showwarning("Brak kolumny 'cat'", 
                                               f"Plik '{os.path.basename(csv_file)}' nie zawiera kolumny 'cat'. "
                                               "Arkusz 'mapowanie' nie zostanie utworzony dla tego pliku.")

                successful_files += 1
                self.status_label.configure(text=f"Przetworzono plik: {os.path.basename(csv_file)}", text_color="blue")
        ...
        final_message = f"Zakończono przetwarzanie. Pomyślnie utworzono {successful_files} plików Excela."
        if failed_files:
            final_message += f"\n\nProblemy z {len(failed_files)} plikami:\n" + "\n".join(failed_files)
            ...
            self.status_label.configure(f"Pomyślnie utworzono wszystkie {successful_files} plików Excela.", text_color="green")
