...
def process_feeds(selected_groups, prefix, output_file_path, status_callback):
    ...
    """
    try:
        # --- Lista zakazanych EAN-ów z API sm-prods (zawsze włączone) ---
        # Produkty, których EAN (gtin) jest na tej liście, są pomijane.
        status_callback("Pobieram listę zakazanych EAN-ów z sm-prods...")
        ...
        headers = [
            ...
            "vatSe", "vatDk", "vatFi",
            "deliverySe", "deliveryDk", "deliveryFi", "shipped_from"
        ]
        ws.append(headers)

        base_url = "https://sm-prods.com/feeds/"
        total_groups = len(selected_groups)
        skipped_forbidden = 0  # licznik odsianych produktów (zakazany EAN)
        ...
        for group_index, group_name in enumerate(selected_groups, start=1):
            ...
            for pid, se_offer in se_offers.items():
                ...
                # FILTR ZAKAZANYCH EAN-ów (forbidden_eans) — pomiń produkt, jeśli EAN na liście.
                ...
                row = [
                    ...
                    6, 6, 6,
                    25, 25, 25.5,
                    "HomeDelivery", "HomeDelivery", "HomeDelivery", "EU"
                ]
                ws.append(row)

        status_callback("Zapisywanie pliku Excel...")
        wb.save(output_file_path)
        info_odsiew = f"\n\nOdsiano produktów z zakazanym EAN: {skipped_forbidden}." if forbidden_eans else ""
        messagebox.showinfo("Sukces", f"Plik '{os.path.basename(output_file_path)}' został pomyślnie zapisany.{info_odsiew}")
