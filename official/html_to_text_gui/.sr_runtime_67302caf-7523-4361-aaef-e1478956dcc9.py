...
QGroupBox {
    border: 1px solid #4a3352; border-radius: 8px; margin-top: 14px;
    padding: 10px 8px 8px 8px; font-weight: 600;
}
QGroupBox::title {
    subcontrol-origin: margin; left: 12px; padding: 0 6px; color: #ff6fb5;
}
QPushButton {
    background: #6d2f5e; border: 1px solid #a04a86; border-radius: 6px;
    padding: 7px 14px; color: #ffffff; font-weight: 600;
}
QPushButton:hover { background: #8c3c78; }
QPushButton:disabled { background: #33293a; color: #7b6f82; border-color: #3d3145; }
QPushButton#primary { background: #c2185b; border-color: #ff4f90; padding: 9px 18px; }
QPushButton#primary:hover { background: #e0246c; }
QLineEdit, QComboBox, QSpinBox, QTextEdit, QTableWidget {
    background: #1f1926; border: 1px solid #43354d; border-radius: 5px;
    padding: 5px; color: #f2e9f2; selection-background-color: #c2185b;
}
...
QProgressBar {
    border: 1px solid #43354d; border-radius: 6px; text-align: center;
    background: #1f1926; height: 20px;
}
QProgressBar::chunk { background: #c2185b; border-radius: 5px; }
...
class MainWindow(QMainWindow):
    ...
    def _build(self) -> None:
        ...
        self.sheet_cmb.setMinimumWidth(180)
        ...
        r = 0
        ol.addWidget(QLabel("Znak listy (<li>):"), r, 0)
        ...
        ol.addWidget(self.bullet_cmb, r, 1)
        r += 1
        self.number_ol_chk = QCheckBox("Numeruj listy <ol> (1. 2. 3.)")
        self.number_ol_chk.setChecked(True)
        self.number_ol_chk.stateChanged.connect(self.refresh_preview)
        ol.addWidget(self.number_ol_chk, r, 0, 1, 2)
        r += 1
        self.blank_chk = QCheckBox("Pusta linia miedzy akapitami")
        self.blank_chk.setChecked(True)
        self.blank_chk.stateChanged.connect(self.refresh_preview)
        ol.addWidget(self.blank_chk, r, 0, 1, 2)
        r += 1
        ol.addWidget(QLabel("Max kolejnych enterow:"), r, 0)
        ...
        ol.addWidget(self.max_nl_spin, r, 1)
        r += 1
        ol.addWidget(QLabel("Separator komorek tabeli:"), r, 0)
        self.cell_sep_edit = QLineEdit(" | ")
        self.cell_sep_edit.textChanged.connect(self.refresh_preview)
        ol.addWidget(self.cell_sep_edit, r, 1)
        r += 1
        self.unescape_chk = QCheckBox("Odkoduj zakodowany HTML (&lt;p&gt;)")
        self.unescape_chk.setChecked(True)
        self.unescape_chk.stateChanged.connect(self.refresh_preview)
        ol.addWidget(self.unescape_chk, r, 0, 1, 2)
        r += 1
        self.leftover_chk = QCheckBox("Wytnij resztki tagow z tresci (</strong >)")
        self.leftover_chk.setChecked(True)
        self.leftover_chk.stateChanged.connect(self.refresh_preview)
        ol.addWidget(self.leftover_chk, r, 0, 1, 2)
        r += 1
        self.alt_chk = QCheckBox("Dopisz tekst alt z <img>")
        self.alt_chk.stateChanged.connect(self.refresh_preview)
        ol.addWidget(self.alt_chk, r, 0, 1, 2)
        r += 1
        ol.addWidget(QLabel("Zapis enterow:"), r, 0)
    ...
    def load_file(self, path: Path, keep_output: bool = False) -> None:
        ...
        try:
            ...
            if self.data is None or self.data.path != path:
                sheet = None
            self.status_lbl.setText("Wczytuje...")
            ...
            self.status_lbl.setText("Gotowe do konwersji.")
        finally:
            self._loading = False
    ...
    def next_html_row(self) -> None:
        ...
        for offset in range(1, len(rows) + 1):
            ...
            if looks_like_html(rows[i].get(col)):
                ...
                return
        self.status_lbl.setText(f"Brak HTML w kolumnie {col}.")
    ...
    def start_convert(self) -> None:
        ...
        self.status_lbl.setText("Konwertuje...")
    ...
    def _on_done(self, stats: dict) -> None:
        ...
        msg += f"\nPlik: {stats['path']}"
        self.status_lbl.setText(f"Gotowe: {stats['path']}")
