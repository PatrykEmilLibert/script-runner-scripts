"""
CDON Product Updater (PUT /v2/articles/bulk) - PySide6
...
UWAGA: ten skrypt, CDON_API_MULTI.py oraz cdon_for_Magda.py tworzą jedną
...
class CDONClient:
    ...
    def _get_headers(self):
        ...
        return {
            ...
            "User-Agent": "CDON-Python-Updater/2.0-Qt"
        }
    ...
    def process_product_data(self, data, stop_event=None):
        """
        Przetwarza jeden wiersz danych i wysyła żądanie do CDON.
        Zwraca True jeśli sukces, False jeśli błąd, None jeśli przerwano.
        """
        sku = data.get('sku')
        if not sku:
            if any(data.values()):
        ...
        if manufacturer: article_body["manufacturer"] = manufacturer
        ...
        if data.get('shipped_from') and str(data['shipped_from']).strip():
            ...
            # Walidacja: shipped_from musi być EU lub NON_EU
            if shipped_from_val in ['EU', 'NON_EU']:
                article_body["shipped_from"] = shipped_from_val
            else:
                self.log(f"[WARN] {sku}: Nieprawidłowy shipped_from '{shipped_from_val}'. Dozwolone: EU, NON_EU")
                            ...
                            if is_success is False:
                                ...
                                if isinstance(msg, list):
                                    msg = "; ".join([
                                        ...
                                        for e in msg if isinstance(e, dict)
                                    ])
                                elif not msg:
                                    msg = json.dumps(item)
...
class CDONUpdaterWindow(QWidget):
    def __init__(self):
        super().__init__()

        self.setWindowTitle("CDON Product Updater v3.0 - Qt / Excel")
        self.resize(900, 850)
    ...
    def _create_widgets(self):
        ...
        header = QLabel("NARZĘDZIE AKTUALIZACJI (WIELOWĄTKOWE)")
        header.setAlignment(Qt.AlignCenter)
        header.setStyleSheet("font-size: 18px; font-weight: bold; color: #ff69b4; padding: 6px;")
        layout.addWidget(header)

        # --- Konto API ---
        account_box = QGroupBox("Konfiguracja połączenia")
        ...
        # --- Akcje ---
        action_box = QGroupBox("Aktualizacja")
        action_layout = QVBoxLayout(action_box)

        self.start_btn = QPushButton("START (ASYNC)")
    ...
    def _set_progress(self, current, total, success):
        self.progress_bar.setRange(0, max(total, 1))
        self.progress_bar.setValue(current)
        self.status_label.setText(f"Postęp: {current}/{total} (OK: {success})")
    ...
    def start_process(self):
        ...
        self.log_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), f"cdon_update_{timestamp}.log")
    ...
    def run_import(self, merchant_id, api_token, num_workers, use_sandbox, file_path, sheet_name):
        ...
        self.log("=" * 80)
        self.log(f"--- START AKTUALIZACJI (Wątków: {num_workers}) ---")
        ...
        try:
            rows = load_table(file_path, sheet_name, self.log)
        except Exception as e:
        ...
        try:
            ...
            def process_row(row_data):
                ...
                result = client.process_product_data(row_data, stop_event=self.stop_event)
                ...
                with lock:
                    ...
                    current = progress_counter["processed"]
                    success = progress_counter["success"]

                self.signals.progress.emit(current, total, success)
                return result
            ...
            self.log(f"Przetworzono: {progress_counter['processed']}/{total} | Sukcesy: {progress_counter['success']} | Błędy: {progress_counter['failed']}")
    ...
    def _on_finished(self, message):
        ...
        self.start_btn.setText("START (ASYNC)")
        self.stop_btn.setEnabled(False)
    ...
    def closeEvent(self, event):
        if self.is_running:
            answer = QMessageBox.question(
                self, "Aktualizacja w toku",
                "Aktualizacja wciąż trwa. Zatrzymać i zamknąć?",
