...
class CDONClient:
    def __init__(self, merchant_id, api_token, use_sandbox=True, log_callback=None):
        ...
        self.base_url = "https://merchants-api.sandbox.fyndiq.se/api/v1" if use_sandbox else "https://merchants-api.fyndiq.se/api/v1"
        self.log = log_callback if log_callback else print
        self.request_count = 0
        self.rate_limit_per_minute = 350
        self._rate_window_seconds = 60
        self._request_timestamps = deque()
        self._rate_lock = threading.Lock()
        self.retry_on_429_wait_seconds = 60
        self.retry_on_429_max_retries = 2
    ...
    def _wait_for_rate_limit(self):
        while True:
            ...
            with self._rate_lock:
                ...
                sleep_for = self._rate_window_seconds - (now - oldest_in_window)
    ...
    def _get_headers(self):
        ...
        return {
            ...
            "User-Agent": "Fyndiq-Python-Importer/1.0-Threaded"
        }
    ...
    def create_product_from_flat_data(self, data):
        """Wysyła jeden produkt metodą POST (Create article)"""
        sku = data.get('sku')
        if not sku:
            if any(data.values()):
        ...
        for suffix, config in market_config.items():
            ...
            if data.get(price_key) and str(data[price_key]).strip():
                ...
                if del_type:
                    ...
                    d_map = {'servicepoint': 'service_point', 'service_point': 'service_point', 'mailbox': 'mailbox'}
                    val = d_map.get(lookup_key)
                    if val:
                        api_delivery.append({"market": code, "value": val})
                    else:
                        self.log(f"[WARN] {sku}: delivery{suffix}='{raw_val}' nie jest wspierane przez Fyndiq (dozwolone: mailbox, service_point).")
        ...
        category_raw = data.get('categories') or data.get('category') or ''
        categories = [c.strip() for c in re.split(r'[;,|]', str(category_raw)) if c.strip()]

        main_image = str(data.get('mainImage', '')).strip()
        if not main_image:
            self.log(f"[BŁĄD DANYCH] {sku}: Brak mainImage (wymagane przez Fyndiq Create article).")
            return False

        if not categories:
            self.log(f"[BŁĄD DANYCH] {sku}: Brak category/categories (Fyndiq wymaga listy categories).")
            return False

        if not api_markets or not api_titles or not api_descriptions or not api_prices or not api_shipping:
            self.log(f"[BŁĄD DANYCH] {sku}: Brak wymaganych danych Fyndiq (markets/title/description/price/shipping_time).")
            return False

        if 'SE' in api_markets and not any(t.get('language') == 'sv-SE' and str(t.get('value', '')).strip() for t in api_titles):
            self.log(f"[WARN] {sku}: Brak tytułu sv-SE dla rynku SE. Fyndiq zwykle wymaga szwedzkiego (chyba że konto jest w programie tłumaczeń).")
        ...
        if api_delivery:
            article_payload["delivery_type"] = api_delivery
        if properties:
            article_payload["properties"] = properties
        if data.get('brand') and str(data.get('brand')).strip():
            article_payload["brand"] = str(data.get('brand')).strip()
        if data.get('gtin') and str(data.get('gtin')).strip():
            article_payload["gtin"] = str(data.get('gtin')).strip()
        ...
        try:
            ...
            while True:
                ...
                if isinstance(response_body, dict):
                    self.log(f"  Response Body: {json.dumps(response_body, ensure_ascii=False)[:1000]}")
                else:
                    self.log(f"  Response Body: {str(response_body)[:1000]}")

                if response.status_code == 429 and attempt < self.retry_on_429_max_retries:
                    wait_seconds = self.retry_on_429_wait_seconds
                    retry_after = response.headers.get("Retry-After")
                    if retry_after:
                        try:
                            retry_after_val = int(float(retry_after))
                            if retry_after_val > wait_seconds:
                                wait_seconds = retry_after_val
                        except ValueError:
                            pass

                    attempt += 1
                    self.log(f"[RATE LIMIT] {sku}: Odpowiedź 429. Czekam {wait_seconds}s i ponawiam próbę ({attempt}/{self.retry_on_429_max_retries}).")
                ...
                if response.status_code in [200, 201]:
                    try:
                        ...
                        if isinstance(response_data, dict) and response_data.get('errors'):
                            self.log(f"[BŁĄD API] {sku}: {json.dumps(response_data.get('errors'), ensure_ascii=False)}")
                            return False

                        article_id = response_data.get('id') if isinstance(response_data, dict) else None
                        if article_id:
                            self.log(f"[OK] {sku}: Utworzono article id={article_id}")
                        else:
                            self.log(f"[OK] {sku}: Wysłano pomyślnie.")
                        return True

                    except json.JSONDecodeError:
                        self.log(f"[OK] {sku}: Status {response.status_code} (brak JSON w odpowiedzi)")
                        return True
                else:
...
class CDONApp(ctk.CTk):
    def __init__(self):
        ...
        self.title("Fyndiq Product Importer v1.0 - Multithreaded Create")
    ...
    def _create_widgets(self):
        ...
        self._setup_log_scroll_bindings()

    def _setup_log_scroll_bindings(self):
        ...
        textbox.bind("<MouseWheel>", _on_mousewheel)
        textbox.bind("<Button-4>", _on_button4)
        textbox.bind("<Button-5>", _on_button5)
    ...
    def start_process(self):
        ...
        self.log_file = f"fyndiq_import_{timestamp}.log"
    ...
    def run_import(self, merchant_id, api_token):
        ...
        self.log("=" * 80)
        self.log("--- START IMPORTU FYNDIQ (WIELOWĄTKOWY) ---")
