...
def _keep_vba_for(filepath):
    """Zwraca True dla plików z makrami (.xlsm), by openpyxl zachował VBA."""
    return os.path.splitext(filepath)[1].lower() == ".xlsm"
...
def podziel_excel(plik_wejsciowy, wierszy_na_plik, folder_wyjsciowy, liczba_wierszy_naglowka=0, sheet_identifier=0, app_instance=None):
    """Dzieli plik Excel, tworząc kopie oryginału i usuwając zbędne wiersze (openpyxl, wieloplatformowo)."""
    try:
        ...
        keep_vba = _keep_vba_for(plik_wejsciowy)
        ...
        for i in range(liczba_plikow_do_utworzenia):
            ...
            try:
                workbook = openpyxl.load_workbook(sciezka_wyjsciowa, keep_vba=keep_vba)
                ws = workbook[sheet_identifier]
                ...
                # Najpierw usuwamy wiersze z dołu (indeksy górnych pozostają wtedy poprawne).
                ostatni_realny = max(liczba_wierszy_df, ws.max_row)
                if ostatni_wiersz_danych_do_zachowania < ostatni_realny:
                    ws.delete_rows(ostatni_wiersz_danych_do_zachowania + 1,
                                   ostatni_realny - ostatni_wiersz_danych_do_zachowania)
                if pierwszy_wiersz_danych_do_zachowania > liczba_wierszy_naglowka + 1:
                    ws.delete_rows(liczba_wierszy_naglowka + 1,
                                   pierwszy_wiersz_danych_do_zachowania - 1 - liczba_wierszy_naglowka)

                workbook.save(sciezka_wyjsciowa)
                workbook.close()
                pliki_utworzone += 1
...
def scal_pliki(base_file_path, sheet_identifier, header_rows_count, app_instance=None):
    """Scala grupę plików w jeden, bazując na pliku wzorcowym i wybranym arkuszu (openpyxl)."""
    ...
    try:
        keep_vba = _keep_vba_for(output_path)
        main_workbook = openpyxl.load_workbook(output_path, keep_vba=keep_vba)
        main_ws = main_workbook[sheet_identifier]

        # Czyścimy dotychczasową zawartość arkusza (zachowując pozostałe arkusze pliku wzorcowego).
        if main_ws.max_row > 0:
            main_ws.delete_rows(1, main_ws.max_row)
        ...
        for r, wiersz in enumerate(data_to_write, start=1):
            for c, wartosc in enumerate(wiersz, start=1):
                main_ws.cell(row=r, column=c, value=wartosc)
